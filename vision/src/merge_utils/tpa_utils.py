...
def load_t5(merging_tasks, model_key, pretrained_state_dict):
    ...
    # load the finetune and pretrained checkpoints
    #filepaths = [os.path.join(load_dir, task, "pytorch_model.bin") for task in merging_tasks]
    filepaths = [os.path.join(load_dir,task,"best.pt") for task in merging_tasks]
    ft_checks = [torch.load(fp) for fp in filepaths]
...
def load_vit(merging_tasks, load_dir):
    ...
    # ptm_check = torch.load(os.path.join(load_dir, "ties_merged.pt")).state_dict()
    ...
    # )
    logger.info(
        f"Pretrained Model:  Mean:{flat_ptm.mean():.6f}\tSTD: {flat_ptm.std():.6f}"
    )
    logger.info(f"Merging tasks: {merging_tasks}")
    logger.info(
        f"Finetune checkpoint \tMean:{flat_ft.mean(dim=1)}\tSTD: {flat_ft.std(dim=1)}"
    ...
    return tv_flat_checks, flat_ptm, flat_ft, \
    ft_checks, ptm_check, remove_keys
